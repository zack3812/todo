...
async function getSession(env, token) {
  ...
  const userId = await env.USER_DB.get(`session:${token}`);
  if (!userId) return null;
  const user = await env.USER_DB.get(`user:${userId}`, 'json');
  return user;
}
...
async function handleLogin(request, env) {
  ...
  let user = await env.USER_DB.get(`user:${employeeId}`, 'json');
  ...
  await env.USER_DB.put(`session:${token}`, employeeId, { expirationTtl: 60 * 60 * 24 * 7 }); // 7天
...
async function handleChangePassword(request, env) {
  ...
  await env.USER_DB.put(`user:${user.employeeId}`, JSON.stringify(user));
...
// 在线心跳：桌面端每 30 秒上报一次，KV 90 秒过期
async function handlePresence(request, env) {
  const user = await requireAuth(env, request);
  if (!user) return json({ error: '未登录' }, 401);
  await env.USER_DB.put(`presence:${user.employeeId}`, String(Date.now()), { expirationTtl: 90 });
  return json({ ok: true, employeeId: user.employeeId });
}

async function isOnline(env, employeeId) {
  const p = await env.USER_DB.get(`presence:${employeeId}`);
  return !!p;
}
...
async function handleListUsers(request, env) {
  ...
  let cursor;
  do {
    const list = await env.USER_DB.list({ prefix: 'user:', ...(cursor ? { cursor } : {}) });
    for (const key of list.keys) {
      const u = await env.USER_DB.get(key.name, 'json');
      if (u) {
        users.push({
          ...
          online: userConnections.has(u.employeeId) && userConnections.get(u.employeeId).size > 0
            || !!(await env.USER_DB.get(`presence:${u.employeeId}`)),
        });
      }
    }
...
async function handleCreateUser(request, env) {
  ...
  const existing = await env.USER_DB.get(`user:${employeeId}`, 'json');
  ...
  await env.USER_DB.put(`user:${employeeId}`, JSON.stringify(user));
...
async function handleAllTodos(request, env) {
  ...
  let cursor;
  do {
    const list = await env.TODO_DB.list({ prefix: 'todo:', ...(cursor ? { cursor } : {}) });
    for (const key of list.keys) {
      const t = await env.TODO_DB.get(key.name, 'json');
      if (t) {
        todos.push({
          ...t,
...
async function handleMyTodos(request, env) {
  ...
  const todos = [];
  const list = await env.TODO_DB.list({ prefix: 'todo:' + user.employeeId + ':' });
  for (const key of list.keys) {
    const t = await env.TODO_DB.get(key.name, 'json');
    if (t) todos.push(t);
  }
...
async function handleTodoSync(request, env) {
  ...
  const key = `todo:${user.employeeId}:${todoId}`;

  if (action === 'delete') {
    await env.TODO_DB.delete(key);
    broadcastToUser(user.employeeId, { event: 'todo.delete', todoId, source: 'cloud' });
    return json({ ok: true });
  }

  const current = await env.TODO_DB.get(key, 'json');
  const clientUpdatedAt = Number(data.clientUpdatedAt);
  if (
    current
    && Number.isFinite(clientUpdatedAt)
    && Number(current.clientUpdatedAt || current.updatedAt) > clientUpdatedAt
  ) {
  ...
  await env.TODO_DB.put(key, JSON.stringify(todo));

  // 广播给同用户其他设备
  broadcastToUser(user.employeeId, {
...
async function handleWebSocket(request, env) {
  ...
  userConnections.get(userId).add(server);
